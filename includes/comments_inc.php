...
if( BitBase::verifyId( $_REQUEST['delete_comment_id'] ?? 0 )) {
...
if( BitBase::verifyId( $_REQUEST['post_comment_id'] ?? 0 ) && $gContent->hasUserPermission( 'p_liberty_post_comments' )) {
...
if( !empty( $_REQUEST['post_comment_submit'] ) && $gContent->hasUserPermission( 'p_liberty_post_comments' )) {
	...
	if( empty( $formfeedback['error'] ) && $storeComment->storeComment( $_REQUEST )) {
		...
		if( empty( $_REQUEST['post_comment_id'] ) && $gBitSystem->isPackageActive( 'switchboard' ) ) {
			...
			// Draft the message:
			$message['subject'] = KernelTools::tra( 'New comment on:' ).' '.$gContent->getTitle().' @ '.$gBitSystem->getConfig( 'site_title' );
			$message['message'] = KernelTools::tra('A new message was posted to ').' '.$gContent->getTitle()."<br/>\n".$gContent->getDisplayUri()."<br/>\n"
...
$gBitSmarty->assign('post_comment_request', $post_comment_request);
...
if( !empty( $_REQUEST['post_comment_preview'] )) {
	...
	$gBitSmarty->assign( 'no_js_preview', $no_js_preview );
...
if( BitBase::verifyId( $_REQUEST['post_comment_reply_id'] ?? 0 )) {
	...
	$comment_prefix = ( preg_match( '/^' . KernelTools::tra( 'Re:' ) . '/', $tmpComment->mInfo['title'] ) ) ? '' : KernelTools::tra( 'Re:' ) . " ";
...
if ($gContent->hasUserPermission( 'p_liberty_read_comments' )) {
	...
	$maxComments = !empty( $_SESSION['liberty_comments_per_page'] ) ? $_SESSION['liberty_comments_per_page'] : $gBitSystem->getConfig( 'comments_per_page', 10 );

	if (!empty( $_REQUEST["comments_maxComments"] )) {
		$maxComments = $_REQUEST["comments_maxComments"];
		$comments_at_top_of_page = true;
		$_SESSION['liberty_comments_per_page'] = $maxComments;
	}

	$comments_sort_mode = ( !empty( $_SESSION['liberty_comments_ordering'] ) ) ? $_SESSION['liberty_comments_ordering'] : $gBitSystem->getConfig( 'comments_default_ordering', 'commentDate_desc' );
	...
	$comments_display_style = ( !empty( $_SESSION['liberty_comments_display_mode'] ) ) ? $_SESSION['liberty_comments_display_mode'] : $gBitSystem->getConfig( 'comments_default_display_mode', 'threaded' );
	...
	if (!BitBase::verifyId( $commentsParentId )) {
		...
		$parents = ( BitBase::verifyId( $commentsParentIds ?? [] ) ) ? $commentsParentIds : $commentsParentId;
	...
	$gBitSmarty->assign( 'comments', $commentsTree );
	$gBitSmarty->assign( 'maxComments', $maxComments );

	$numCommentPages = ceil( $numComments / $maxComments );
	$comments_return_url .= !strpos( $comments_return_url, '?' ) ? '?' : '';
	...
	$gBitSmarty->assign( 'gComment', $gComment );

	$gBitSmarty->assign( 'currentTimestamp', time() );
	$gBitSmarty->assign( 'comments_return_url', $comments_return_url );
	$gBitSmarty->assign( 'comments_at_top_of_page', ( isset( $comments_at_top_of_page ) && $gBitSystem->getConfig( 'comments_reorganise_page_layout', 'n' ) == 'y' ) ? $comments_at_top_of_page : false );
//	$comments_yelurneurl .= ( !strp,s(mments_dispye_urnsurl, '?' ) ? '?' : '' )
